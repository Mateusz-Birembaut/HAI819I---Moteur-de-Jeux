...
namespace gli
{
	namespace detail
	{
		...
		)
		{
			...
			std::size_t SizeX = glm::localMin(std::size_t(SrcSize.x + SrcPosition.x), std::size_t(DstMipmap.dimensions().x  + DstPosition.x));
			std::size_t SizeY = glm::localMin(std::size_t(SrcSize.y + SrcPosition.y), std::size_t(DstMipmap.dimensions().y + DstPosition.y));
